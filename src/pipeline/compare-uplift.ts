...
async function runOne(spec: CaseSpec): Promise<void> {
  ...
  // Resolve once, keyed by NORMALIZED citation (what verifiability's predicate sees).
  const tacetKeys = [...new Set(tacetCites.map(normalizeCitation))];
  const baseKeys = [...new Set(baseCites.map(normalizeCitation))];
  const tOk = await mapPool(tacetKeys, 8, (k) => resolves(toUrl(k)));
  const bOk = await mapPool(baseKeys, 8, (k) => resolves(toUrl(k)));
  const tacetSet = new Set(tacetKeys.filter((_, i) => tOk[i]));
  const baseSet = new Set(baseKeys.filter((_, i) => bOk[i]));
  ...
  const comparison = {
    ...
    measurements: {
      verifiability: {
        tacet: verifiability(tacetCites, (c) => tacetSet.has(c)),
        baseline: verifiability(baseCites, (c) => baseSet.has(c)),
      },
  ...
  console.log(`[${spec.case}] verifiability TACET=${m.verifiability.tacet.fraction} baseline=${m.verifiability.baseline.fraction} | abstentions=${abstentions.total} hedges=${m.uncertainty.baseline.hedges} verdicts=${m.uncertainty.baseline.verdicts} | hidden idMatches=${hidden.idMatches.length} names=${hidden.nameMentions.join("/")} → ${outPath}`);
}
